# Databricks notebook source exported at Thu, 30 Jun 2016 12:24:02 UTC
# MAGIC %md
...
logs_sum_df = (not200DF.groupBy('path').count().sort(col('count').desc()))
...
day_group_hosts_df = day_to_host_pair_df.distinct().groupBy('day')
# day_group_hosts_df.show(10, False)
daily_hosts_df = day_group_hosts_df.count()
...
days_with_hosts = [row['day'] for row in daily_hosts_df.select('day').collect()]
# tmp = daily_hosts_df.select('day').collect()
# print tmp
# print type(tmp[0])
hosts = [row['count'] for row in daily_hosts_df.select('count').collect()]
# for <FILL IN>:
#   <FILL IN>
...
df1 = total_req_per_day_df.select(['day', col('count').alias('requests')])
df2 = daily_hosts_df.select([col('day').alias('day2'), col('count').alias('nHosts')])                                  
avg_daily_req_per_host_df = (df1
                             .join(df2, df1['day'] == df2['day2'], 'outer')
                             .select(['*', (col('requests') / col('nHosts')).alias('avg_reqs_per_host_per_day')])
                             )
# avg_daily_req_per_host_df = avg_daily_req_per_host_df.select(list(set(avg_daily_req_per_host_df.columns)))
...
# print avg_daily_req_per_host_df.columns
# print set(avg_daily_req_per_host_df.columns)
# print list(set(avg_daily_req_per_host_df.columns))
...
days_with_avg = [row['day'] for row in avg_daily_req_per_host_df.select('day').collect()]
...
top_20_not_found_df = not_found_paths_df.limit(20)
...
hosts_404_count_df = not_found_df.groupBy('host').count().sort(desc('count')).limit(25)
...
days_with_errors_404 = [row['day'] for row in errors_by_date_sorted_df.select('day').collect()]
...
top_err_date_df = errors_by_date_sorted_df.sort(desc('count')).limit(5)
...
hours_with_not_found = [row['hour'] for row in hour_records_sorted_df.select('hour').collect()]
