# Databricks notebook source exported at Fri, 24 Jun 2016 12:36:27 UTC
# MAGIC %md
...
pluralLengthsDF = pluralDF.select(length(pluralDF.word).alias('wordLength'))
...
#     assert(isinstance(wordListDF, pyspark.sql.dataframe.DataFrame))
    assert(str(type(wordListDF)) == "<class 'pyspark.sql.dataframe.DataFrame'>")
    assert('word' in wordListDF.columns)
    
    wordCountDF = wordListDF.select('word').groupBy('word').count()
...
print(shakespeareDF.groupBy().count().collect())
