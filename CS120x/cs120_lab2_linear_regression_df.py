# Databricks notebook source exported at Thu, 21 Jul 2016 19:48:09 UTC
# MAGIC %md
...
raw_data_df.show()
# raw_data_df.select(raw_data_df['*']).show()
# dir(parsed_points_df)
# print parsed_points_df.first()

# COMMAND ----------

# print '2001.0,0.884,0.610,0.600,0.474,0.247,0.357,0.344,0.33,0.600,0.425,0.60,0.419'.split(',')
# print split('2001.0,0.884,0.610,0.600,0.474,0.247,0.357,0.344,0.33,0.600,0.425,0.60,0.419', ',')
# print [1, 2, 3, 4, 5][0]
# print [1, 2, 3, 4, 5][-1]
from pyspark.sql import Row
tstRow = Row(split=[u'2001.0', u'0.884123733793', u'0.610454259079', u'0.600498416968', u'0.474669212493', u'0.247232680947', u'0.357306088914', u'0.344136412234', u'0.339641227335', u'0.600858840135', u'0.425704689024', u'0.60491501652', u'0.419193351817'])
print tstRow
print type(tstRow[0])
print type(float(tstRow[0][0]))
print float(tstRow[0][0])
print tstRow[0][1:]
print [float(x) for x in tstRow[0][1:]]

from pyspark.mllib.regression import LabeledPoint
tstLPt = LabeledPoint(float(tstRow[0][0]), [float(x) for x in tstRow[0][1:]])
print tstLPt
print tstLPt.label
print tstLPt.features
...
#     lptDf = df.select(df['*']).flatMap(lambda ln: ln.split())        
#     lptDf = df.flatMap(lambda ln: ln.split())

#     u_split = udf(split)
#     lptDf = df.select(u_split(df['*']))  

#     lptDf = df.map(lambda ln: ln.split(',')).toDF()
  
    obsDf = df.select(sql_functions.split(df['value'], ','))
  
#     print "parse_points: obsDf:"
#     print obsDf
#     print obsDf.count()
#     print obsDf.take(5)
    
    lptDf = obsDf.map(lambda rowLst: LabeledPoint(float(rowLst[0][0]), [float(x) for x in rowLst[0][1:]])).toDF()
#     print "parse_points: lptDf:"    
#     print lptDf
#     print lptDf.count()
#     print lptDf.take(5)    
...
# print "type(parsed_points_df): %s" % (type(parsed_points_df)) 
...
print '\n{0}'.format(parsed_points_df.first())
...
# TODO: Replace <FILL IN> with appropriate code
preds_and_labels_train = parsed_train_data_df.map(lambda lp: (average_train_year, lp.label))
...
preds_and_labels_val = parsed_val_data_df.map(lambda lp: (average_train_year, lp.label))
...
preds_and_labels_test = parsed_test_data_df.map(lambda lp: (average_train_year, lp.label))
...
def gradient_summand(weights, lp):
    ...
    """
    assert isinstance(weights, DenseVector) or isinstance(weights, np.ndarray), \
      "type(weights): %s; expecting DenseVector or numpy.ndarray" % (type(weights))
    assert isinstance(lp, LabeledPoint) or isinstance(lp, Row), \
      "type(lp): %s; expecting LabeledPoint or Row" % (type(lp))
    
    retObj = (weights.dot(lp.features) - lp.label) * lp.features
    return retObj
...
def get_labeled_prediction(weights, observation):
    ...
    """
    assert isinstance(weights, np.ndarray), "type(weights): %s; expecting numpy.ndarray" % (type(weights))
    assert isinstance(observation, LabeledPoint) or isinstance(observation, Row), \
      "type(observation): %s; expecting numpy.ndarray or Row" % (type(observation))
    
    retObj = (float(weights.dot(observation.features)), float(observation.label))
    return retObj
...
assert isinstance(parsed_train_data_df, DataFrame), \
  "type(parsed_train_data_df): %s; expecting pyspark.sql.dataframe.DataFrame" % (type(parsed_train_data_df))
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    """
    assert isinstance(train_data, RDD) or isinstance(train_data, DataFrame), \
      "type(train_data): %s; expecting RDD or pyspark.sql.dataframe.DataFrame" % (type(train_data))
    assert isinstance(train_data.take(1)[0], LabeledPoint) or isinstance(train_data.take(1)[0], Row), \
      "type(train_data.take(1)[0]): %s; expecting LabeledPoint or Row)" % (type(train_data.take(1)[0]))
    ...
    for i in range(num_iters):
        ...
        preds_and_labels_train = train_data.map(lambda obs: get_labeled_prediction(w, obs))
#         print "type(preds_and_labels_train): %s" % (type(preds_and_labels_train))
#         print "preds_and_labels_train.take(5): %s" % (preds_and_labels_train.take(5))        
        
        preds_and_labels_train_df = sqlContext.createDataFrame(preds_and_labels_train, ["prediction", "label"])
#         schema = StructType([StructField("prediction", FloatType(), True), 
#                              StructField("label"     , FloatType(), True)])
#         preds_and_labels_train_df = sqlContext.createDataFrame(preds_and_labels_train, schema)
#         print "type(preds_and_labels_train_df): %s" % (type(preds_and_labels_train_df))
#         print "preds_and_labels_train_df.take(5): %s" % (preds_and_labels_train_df.take(5))        
        ...
        gradient = train_data.map(lambda obs: gradient_summand(w, obs)).reduce(lambda obs1, obs2: obs1 + obs2)
#         print "type(gradient): %s" % (type(gradient))
#         print "gradient.take(5): %s" % (gradient.take(5)) # when gradient is an RDD
...
def two_way_interactions(lp):
    ...
    interactFtr = [x * y for x, y in itertools.product(lp.features, lp.features)]
#     print "two_way_interactions: lp.features: %s" % (lp.features)
#     print "two_way_interactions: interactFtr: %s" % (interactFtr)    
    retObj = LabeledPoint(lp.label, np.hstack((lp.features, interactFtr)))
    return retObj
