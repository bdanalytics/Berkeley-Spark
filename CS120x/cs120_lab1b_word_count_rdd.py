# Databricks notebook source exported at Fri, 15 Jul 2016 13:38:08 UTC
# MAGIC %md
...
# print wordsGrouped.take(1)

# COMMAND ----------

# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = (wordsGrouped
                     .map(lambda (word, knt) : (word, list(knt)))
                     .map(lambda (word, kntLst) : (word, sum(kntLst)))     
                    )
...
wordCounts = wordPairs.reduceByKey(lambda knt1, knt2 : knt1 + knt2)
...
totalCount = (wordCounts
              .map(lambda (word, knt) : knt)
              .reduce(lambda knt1, knt2 : knt1 + knt2))
...
import pyspark.rdd
# print type(wordsRDD)
# print isinstance(wordsRDD, pyspark.rdd.RDD)
# print isinstance(shakeWordsRDD, pyspark.rdd.RDD)
...
def wordCount(wordListRDD):
    ...
    """
    #     assert type(wordListRDD) == type(sc.parallelize(['cat', 'elephant', 'rat', 'rat', 'cat'], 4)), 
    assert isinstance(wordListRDD, pyspark.rdd.RDD), \
            "unknown wordListRDD type: {0}".format(type(wordListRDD))
...
def removePunctuation(text):
    ...
    """
#     assert isinstance(text, str), "unknown text type: {0}".format(type(text))
    textLower = text.lower()
    textNoPunct = re.sub(r"[^a-zA-Z0-9 ]", "", textLower)
    return textNoPunct.strip()
    
print removePunctuation('Hi, you!')
print removePunctuation(' No under_score!')
print removePunctuation(' *      Remove punctuation then spaces  * ')
# print removePunctuation(3)
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, key = lambda (wrd, knt) : -knt)
