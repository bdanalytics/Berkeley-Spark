# Databricks notebook source exported at Sun, 4 Sep 2016 00:38:03 UTC
# MAGIC %md
...
amazonRecToToken = amazonSmall.map(lambda (id, txt): (id, tokenize(txt)))
googleRecToToken = googleSmall.map(lambda (id, txt): (id, tokenize(txt)))
...
def countTokens(vendorRDD):
    ...
    """
#     print "countTokens: vendorRDD.take(3):%s" % (vendorRDD.take(3))
#     print "countTokens: vendorRDD.map.take(3):%s" % (vendorRDD.map(lambda tpl: len(tpl[1])).take(3))    
    
#     return 0
    return vendorRDD.map(lambda tpl: len(tpl[1])).reduce(lambda a, b: a + b)
#     return vendorRDD.reduce(lambda tplA, tplB: len(tplA[1]) + len(tplB[1]))  
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, key = lambda (id, tokenLst): -len(tokenLst))
...
def tf(tokens):
    ...
    retDct = {}
    for token in tokens:
      if retDct.has_key(token):
        retDct[token] += 1. / len(tokens)
      else:
        retDct[token]  = 1. / len(tokens)
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = sc.union([amazonRecToToken, googleRecToToken])
...
def idfs(corpus):
    ...
    """
#     print "idfs: corpus.take(3): %s" % (corpus.take(3))
#     uniqueTokens = corpus.flatMap(lambda (id, tokens): set(tokens))
#     print "idfs: uniqueTokens.take(3): %s" % (uniqueTokens.take(3))
#     tokenCountPairTuple = uniqueTokens.map(lambda token: (token, sum(corpus.map(lambda tokenLst: token in tokenLst))))
#     print "idfs: tokenCountPairTuple.take(3): %s" % (tokenCountPairTuple.take(3))
...
#     return corpus
    ...
    N = corpus.count()
    retRDD = (corpus
                .flatMapValues(lambda (token): token)
                .distinct()
                .map(lambda(id, token): (token, 1))
                .reduceByKey(lambda a,b: a + b)
                .map(lambda(token, knt): (token, N * 1.0 / knt)))
    return (retRDD)
...
# idfsToCountRow = idfsSmall.map(lambda (token, idf): (idf, 1)).groupByKey().count()
idfsToCountRow = idfsSmall.map(lambda (token, idf): (idf, 1)).groupByKey().mapValues(len)
# print idfsToCountRow.take(3)
...
def tfidf(tokens, idfs):
    ...
    tfs = tf(tokens)
    tfIdfDict = tfs
    for token in tfIdfDict:
      tfIdfDict[token] = tfIdfDict[token] * idfs[token]
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    commonTokens = [token for token in a if b.has_key(token)]
#     print commonTokens
    retval = 0
    for token in commonTokens:
        retval = retval + 1.0 * a[token] * b[token]
...
def cossim(a, b):
    ...
    """
    retval = dotprod(a, b) / (norm(a) * norm(b))  
    return retval
...
sims = (similaritiesBroadcast
          .map(lambda (googleURL, amazonID, cs): (amazonID + ' ' + googleURL, cs)))

trueDupsRDD = (sims
                .join(goldStandard)
                .map(lambda(comboID, valueTpl): (valueTpl[0]))
              )
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.mean()

nonDupsRDD = (sims
               .leftOuterJoin(goldStandard)
               .filter(lambda(comboID, valueTpl): valueTpl[1] == None)
               .map(lambda(comboID, valueTpl): (valueTpl[0]))             
             )
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = sc.union([amazonFullRecToToken, googleFullRecToToken])
...
amazonWeightsRDD = (amazonFullRecToToken
                    .map(lambda(id, tokens): (id, tfidf(tokens, idfsFullWeights))))
...
amazonNorms = (amazonWeightsRDD
               .map(lambda(id, tfidfDict): (id, norm(tfidfDict)))
               .collectAsMap()
              ) 
...
def invert(record):
    ...
    id = record[0]
    tokensDct = record[1]
    pairs = [(token_key, id) for token_key in tokensDct]
    return (pairs)
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([1.0 * amazonWeightsBroadcast.value[amazonRec][token] * 
                   googleWeightsBroadcast.value[googleRec][token] for token in tokens])
    value = 1.0 * s / (amazonNorms[amazonRec] * googleNorms[googleRec])
