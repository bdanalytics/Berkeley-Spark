# Databricks notebook source exported at Sat, 3 Sep 2016 09:42:54 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
pluralRDD = wordsRDD.map(lambda word: makePlural(word))
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (k,vLst): (k, sum(vLst)))
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda word: (word, 1)).groupByKey().count()
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    assert isinstance(wordListRDD, RDD), \
      "wordCount: type(wordListRDD): %s; expecting RDD" % (type(wordListRDD))
...
def removePunctuation(text):
    ...
    assert isinstance(text, str) or isinstance(text, unicode), \
      "removePunctuation: type(text): %s; expecting str or unicode only" % (type(text))
    
    textNoPunct = re.sub("[^a-zA-Z0-9 ]", "", text)
    textLowerCase = textNoPunct.lower()
    textTrimmed = textLowerCase.strip()
